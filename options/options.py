...
def parse(opt_path, is_train=True):
    ...
    with open(opt_path, 'r') as f:
        for line in f:
            line = line.split('//')[0] + '\n'
            json_str += line
    opt = json.loads(json_str, object_pairs_hook=OrderedDict, strict = False)
...
def check_resume(opt):
    ...
    if opt['path']['resume_state']:
        ...
        opt['path']['pretrain_model_G1'] = osp.join(opt['path']['models'],
                                                   '{}_G1.pth'.format(state_idx))
        logger.info('Set [pretrain_model_G] to ' + opt['path']['pretrain_model_G1'])

        opt['path']['pretrain_model_G2'] = osp.join(opt['path']['models'],
                                                   '{}_G2.pth'.format(state_idx))
        logger.info('Set [pretrain_model_G2] to ' + opt['path']['pretrain_model_G2'])
        
        opt['path']['pretrain_model_D1'] = osp.join(opt['path']['models'],
                                                   '{}_D1.pth'.format(state_idx))
        logger.info('Set [pretrain_model_D] to ' + opt['path']['pretrain_model_D1'])
