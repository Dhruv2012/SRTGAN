...
class ICPR_model(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        l_g_gan = self.l_gan_w * (self.cri_gan(HR_D - torch.mean(SR_D), False) +
                                      self.cri_gan(SR_D - torch.mean(HR_D), True)) / 2
        ...
        l_d_real = self.cri_gan(HR_D - torch.mean(SR_D), True)
        l_d_fake = self.cri_gan(SR_D - torch.mean(HR_D), False)
