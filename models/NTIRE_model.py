...
class NTIRE_model(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.netD = networks.define_patchD(opt=opt).to(self.device)
    ...
    def optimize_parameters(self, step):
        ...
        self.SR, self.edge_scores = self.netG(self.var_L)
        self.SR_Encoded = self.netV(self.SR)
        self.HR_Encoded = self.netV(self.var_H)
        self.SR_D = torch.mul(self.netD(self.SR), self.edge_scores)
    ...
    def load(self):
        ...
        load_path_Q = "G:/code/code/latest_G.pth"
