...
class ICPR_model(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        #Quality_loss = 5e-6 * (5-torch.mean(self.netQ(self.SR).detach()))
        ...
        l_g_gan = self.l_gan_w * (self.cri_gan(self.HR_D - torch.mean(self.SR_D), False) +
                                      self.cri_gan(self.SR_D - torch.mean(self.HR_D), True)) / 2
        ...
        l_d_real = self.cri_gan(self.HR_D - torch.mean(self.SR_D), True)
        l_d_fake = self.cri_gan(self.SR_D - torch.mean(self.HR_D), False)

        l_d_total = (l_d_real + l_d_fake) / 2

        #g1 = self.l_gan_w * self.cri_gan(self.SR_D, False)
        #g2 = self.l_gan_w * self.cri_gan(self.HR_D, True)
        ...
        self.log_dict['l_g_vae'] = l_g_vae.item()
        #self.log_dict['l_g_qual'] = Quality_loss.item()
    ...
    def load(self):
        ...
        #load_path_G = "/media/ml/Data Disk/Kalpesh/NTIRE/experiments/ICPR_VAE_DEGRADE_158000/models/158000_G.pth"
        if load_path_G is not None:
            logger.info('Loading pretrained model for G [{:s}] ...'.format(load_path_G))
            self.load_network(load_path_G, self.netG)
        
        load_path_D = None #"/media/ml/Data Disk/Kalpesh/NTIRE/experiments/ICPR_VAE_DEGRADE_158000/models/158000_D.pth"
        if self.opt['is_train'] and load_path_D is not None:
            logger.info('Loading pretrained model for D [{:s}] ...'.format(load_path_D))
            self.load_network(load_path_D, self.netD)

        #load_path_V = "/media/ml/Data Disk/Kalpesh/NTIRE/experiments/ICPR_VAE_DEGRADE_158000/models/158000_V.pth"
        #if self.opt['is_train'] and load_path_V is not None:
        #    logger.info('Loading pretrained model for V [{:s}] ...'.format(load_path_V))
        #    self.load_network(load_path_V, self.netV)
